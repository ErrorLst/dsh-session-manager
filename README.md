...
| 手动批量删除 | “批量删除（> N 天：M 个）”按钮，两步确认后一次性删除「最近更新」超过 `maxAgeDays` 天的所有会话，**过程实时显示进度条与已删除/失败计数**，完成后显示删除/跳过统计 |
| 阈值可配置 | 批量删除阈值（天）、会话数上限（个）、日志大小上限（MB）三值在设置页直接编辑保存，**立即生效**（持久化于 `~/.dsh/storages/dsh_session_manager.json`）；包内 `config.json` 仅作初始基线，「恢复默认」一键回基线 |
...
**设置页可配置（推荐）**：侧栏设置 → 会话管理 → 三个输入框 + 保存。保存在宿主 `~/.dsh/storages/dsh_session_manager.json`（storage 域 `dsh_session_manager`），立即生效，重启后保持。

**包内 `config.json`（仅初始基线）**：首次使用 / 尚未在设置页保存时生效；「恢复默认」也回到这里的值。
...
| 字段 | 默认 | 说明 |
|---|---|---|
| `maxAgeDays` | `30` | 批量删除阈值：“最近更新”早于该天数即视为过期（1~36500） |

...
//        - 手动批量删除：删除「最近更新」早于 config.json maxAgeDays 天的所有会话
//        - 阈值统计展示：总会话数 / 总会话日志大小 与告警上限对比
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SessionsPanel() {
      ...
      var policyLine = [];
      if (maxAgeDays > 0) {
        policyLine.push(React.createElement("span", { key: "age" },
          "批量删除阈值：超过 " + maxAgeDays + " 天未更新"));
      }
      if (maxSessions > 0) {
        policyLine.push(React.createElement("span", { key: "count" },
          "会话数上限 " + maxSessions + " 个"));
      }
      if (maxTotalSizeMB > 0) {
        policyLine.push(React.createElement("span", { key: "size" },
          "日志大小上限 " + fmtSize(maxTotalSizeMB * MIB)));
      }
      ...
      return React.createElement("div", { id: "dsh-session-manager-settings", style: sectionStyle },
        ...
        React.createElement("div", { style: cardStyle },
          React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 6 } },
            ...
            React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: "4px 12px", fontSize: 12, lineHeight: "18px", color: "var(--dsw-alias-label-tertiary)" } }, policyLine)
          ),
          ...
          overCount || overSize ? React.createElement("p", { style: errorStyle }, "超出告警阈值，建议清理过期会话；阈值在包内 config.json 调整。") : null
        ),
        ...
        React.createElement("p", { style: hintStyle },
          "说明：删除会移除该会话的持久化记录（仅日志文件与空会话目录），不会触碰工作区文件；打开中或运行中的会话不可删除。批量删除阈值与告警上限在包内 config.json 调整（maxAgeDays / maxSessions / maxTotalSizeMB），改后重启生效。")
      );
    }

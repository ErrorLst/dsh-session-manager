...
//        - 每行手动删除（两步确认；打开中/运行中的会话不可删，子代理须父会话已关闭）
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var PRUNE_API = "/api/dsh-session-manager/prune";
    var SUMMARY_API = "/api/dsh-session-manager/summary";
    ...
    // ---- 本地缓存：缓存优先显示 + 后台刷新（dsh-notes 同款模式）----
    // 条目形状：{ at, config, stats, sessions? }；sessions 只在全量 GET 后写入，
    // summary 写入保留已有 sessions（无则省略），面板仅在 sessions 存在时用缓存。
    var CACHE_KEY = "dsh-session-manager.cache.v1";
    function cacheRead() {
      try {
        ...
        if (data !== null && typeof data === "object"
          && (Array.isArray(data.sessions) || data.config !== null || data.stats !== null)) {
          return {
            at: typeof j.at === "number" ? j.at : 0,
            config: data.config !== null && typeof data.config === "object" ? data.config : null,
    ...
    function cacheWriteFull(data) {
      try {
        window.localStorage.setItem(CACHE_KEY, JSON.stringify({
          at: Date.now(),
          data: { config: data.config, stats: data.stats, sessions: data.sessions },
        }));
      } catch { /* 存储不可用：仅内存视图 */ }
    }
    function cacheWriteSummary(config, stats) {
      try {
        var prev = cacheRead();
        window.localStorage.setItem(CACHE_KEY, JSON.stringify({
          at: Date.now(),
          data: {
            config: config,
            stats: stats,
            sessions: prev !== null && prev.sessions !== null ? prev.sessions : undefined,
          },
        }));
      } catch { /* 存储不可用：仅内存视图 */ }
    }
    function fetchSummary() {
      return fetch(SUMMARY_API, { cache: "no-store" })
        .then(function (res) { return res.json(); });
    }
    ...
    function SessionsPanel() {
      var stateTuple = React.useState({ status: "loading", sessions: [], config: null, stats: null, error: null, refreshing: false, cacheAt: null });
      var st = stateTuple[0];
      ...
      var pruneTimer = React.useRef(null);
      ...
      // 草稿同步去重：仅配置值真正变化时才重置草稿（后台刷新同值不打断编辑）
      var lastConfigJsonRef = React.useRef(null);

      // 后台加载：成功则替换并写缓存；失败保留当前数据（缓存/旧数据不丢）
      var load = function () {
        setState(function (prev) { return Object.assign({}, prev, { refreshing: true, error: null }); });
        fetchList().then(function (data) {
          if (data && data.ok === true) {
            ...
            setState(function (prev) { return Object.assign({}, prev, { refreshing: false, error: (data && data.error) || "load-failed" }); });
          }
        }).catch(function (error) {
          setState(function (prev) { return Object.assign({}, prev, { refreshing: false, error: String(error) }); });
        });
      };
      ...
      React.useEffect(function () {
        ...
        if (cached !== null && cached.sessions !== null) {
          setState({
            status: "ready",
      ...
      // 服务器配置到达/变化后同步编辑草稿（仅值变化时，避免后台刷新覆盖输入）
      var configJson = st.config === null || st.config === undefined ? null : JSON.stringify(st.config);
      React.useEffect(function () {
        if (configJson === null) return;
        if (lastConfigJsonRef.current === configJson) return;
      ...
      function armPrune() {
        ...
        if (prune.armed) {
          clearArmTimer();
          setPrune({ busy: true, armed: false, done: 0, total: 0, deleted: 0, failed: 0, message: null, error: null });
          // 轮询宿主进度（每删一个更新一次）
          pruneTimer.current = setInterval(function () {
            fetch(PRUNE_API, { cache: "no-store" })
              .then(function (res) { return res.json(); })
              .then(function (data) {
                if (data && data.ok === true && data.prune) {
                  setPrune(function (prev) {
                    if (!prev.busy) return prev;
                    return {
                      busy: true, armed: false,
                      done: data.prune.done, total: data.prune.total,
                      deleted: data.prune.deleted, failed: data.prune.failed,
                      message: null, error: null,
                    };
                  });
                }
              })
              .catch(function () { /* 进度拉取失败静默，下个周期重试 */ });
          }, 400);
          post({ action: "delete-expired" }).then(function (result) {
            if (pruneTimer.current !== null) { clearInterval(pruneTimer.current); pruneTimer.current = null; }
            load();
            if (result && result.ok === true) {
              var r = result.result;
              var skipText = "";
              ...
              setPrune({ busy: false, armed: false, done: 0, total: 0, deleted: 0, failed: 0, message: "已删除 " + r.deleted + " 个过期会话" + skipText, error: null });
            } else {
              setPrune({ busy: false, armed: false, done: 0, total: 0, deleted: 0, failed: 0, message: null, error: (result && result.error) || "batch-failed" });
            }
          });
          return;
        }
        clearArmTimer();
        setPrune({ busy: false, armed: true, done: 0, total: 0, deleted: 0, failed: 0, message: null, error: null });
        armTimer.current = setTimeout(function () {
          armTimer.current = null;
          setPrune({ busy: false, armed: false, done: 0, total: 0, deleted: 0, failed: 0, message: null, error: null });
        }, CONFIRM_TTL_MS);
      }
      ...
      var rowsEl = rows.map(function (row) {
        // 子代理会话额外要求父会话已关闭（parentClosed）才可删，与批量删除策略一致
        var deletable = !row.live && !row.running && (row.origin !== "subagent" || row.parentClosed === true);
        ...
        // 冷子代理但父会话仍打开：该行唯一不可删原因是父未关闭（tooltip 用）
        var parentOpenSubagent = !row.live && !row.running && row.origin === "subagent" && row.parentClosed !== true;
        ...
        return React.createElement("div", {
          ...
          React.createElement("button", {
            ...
            title: deletable
              ? "永久删除该会话（记录与日志）"
              : parentOpenSubagent ? "父会话仍打开，子代理会话不可删除" : "打开中/运行中的会话不可直接删除",
      ...
      return React.createElement("div", { id: "dsh-session-manager-settings", style: sectionStyle },
        ...
        React.createElement("div", { style: cardStyle },
          st.cacheAt !== null
            ? React.createElement("p", { key: "cache", style: Object.assign({}, hintStyle, { margin: 0 }) },
                "显示缓存数据（" + fmtTime(st.cacheAt) + "）· 正在后台刷新…")
            : null,
          ...
          prune.busy ? React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 5 } },
            (function () {
              var pct = prune.total > 0 ? Math.max(0, Math.min(100, Math.round(prune.done / prune.total * 100))) : 0;
              var text = prune.total > 0
                ? "正在批量删除… " + prune.done + " / " + prune.total + "（已删除 " + prune.deleted + (prune.failed > 0 ? " · 失败 " + prune.failed : "") + "）"
                : "正在批量删除… 扫描会话中";
              return [
                React.createElement("div", { key: "line", style: { display: "flex", justifyContent: "space-between", gap: 8, fontSize: 12, lineHeight: "18px", color: "var(--dsw-alias-label-secondary)" } },
                  React.createElement("span", null, text),
                  React.createElement("span", { style: { fontVariantNumeric: "tabular-nums" } }, prune.total > 0 ? pct + "%" : "")
                ),
                React.createElement("div", { key: "bar", style: { height: 6, borderRadius: 3, background: "var(--dsw-alias-interactive-bg-hover)", overflow: "hidden" } },
                  React.createElement("div", { style: { height: "100%", width: pct + "%", background: "var(--dsw-alias-brand-primary)", borderRadius: 3, transition: "width 0.25s ease" } })
                )
              ];
            })()
          ) : null,
    ...
    function WarningOverlay() {
      ...
      // 超限判定：config/stats → {over} 布尔
      function overOf(config, stats) {
        var maxSessions = typeof config === "object" && config !== null && typeof config.maxSessions === "number" ? config.maxSessions : 0;
        var maxMB = typeof config === "object" && config !== null && typeof config.maxTotalSizeMB === "number" ? config.maxTotalSizeMB : 0;
        var count = typeof stats === "object" && stats !== null && typeof stats.count === "number" ? stats.count : 0;
        var sizeBytes = typeof stats === "object" && stats !== null && typeof stats.totalSizeBytes === "number" ? stats.totalSizeBytes : 0;
        return {
          over: (maxSessions > 0 && count > maxSessions) || (maxMB > 0 && sizeBytes > maxMB * MIB),
          count: count,
          sizeBytes: sizeBytes,
        };
      }
      ...
      React.useEffect(function () {
        ...
        var timer = null;
        // 视图状态更新（遵守用户已关闭弹窗）
        function applyState(config, stats) {
          if (dismissedRef.current) return;
          var v = overOf(config, stats);
          if (v.over) {
            setState({ checked: true, show: true, count: v.count, sizeBytes: v.sizeBytes, config: config, error: null });
          } else {
        ...
        var cached = cacheRead();
        if (cached !== null) {
          // 缓存优先：立即按缓存判定；随后 summary 后台核实（修正/更新）
          applyState(cached.config, cached.stats);
          fetchSummary().then(function (data) {
            if (data && data.ok === true) {
              cacheWriteSummary(data.config, data.stats);
              applyState(data.config, data.stats);
            }
          }).catch(function () { /* 核实失败：保留缓存判定 */ });
        } else {
          // 首次（无缓存）：拉轻量摘要；失败 2.5s 后重试一次
          var tried = false;
          var attempt = function () {
            ...
            fetchSummary().then(function (data) {
              if (data && data.ok === true) {
                ...
                setState({ checked: true, show: false, count: 0, sizeBytes: 0, config: null, error: (data && data.error) || "summary-failed" });
              }
            }).catch(function () {
      ...
      return React.createElement("div", {
        ...
        React.createElement("div", {
          ...
          React.createElement("p", { style: { margin: 0, fontSize: 13, lineHeight: "20px", color: "var(--dsw-alias-label-secondary)" } },
            "建议前往 设置 → 会话管理，手动批量删除超过 " + maxAgeDays + " 天未更新的会话，或在上方调整告警上限。"),

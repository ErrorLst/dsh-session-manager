...
//      全量会话快照 + 阈值统计（总数 / 总大小 / 已逾期货），供设置页与启动弹窗使用。
//        POST /api/dsh-session-manager            -> { action, … }
//      动作：
//        delete        删除一个「冷」会话（非打开中、非运行中）的全部持久化工件
//                      —— JSONL 日志文件 + 空会话目录；成功后 emits
//                      `api-session/removed`，让 Web 端会话列表即时移除该行。
//        delete-expired  手动批量删除：删除「最近更新」早于 config.json 中
//                      maxAgeDays 天的所有会话（跳过打开中 / 运行中 / 子代理），
//                      返回删除与跳过统计。
...
export const inject = ['webServer', 'sessions', 'sessionPersistence', 'agents']
...
/* ------------------------------------------------------------------ */
/* 配置：包内 config.json（缺省/损坏时回落内置默认）                       */
...
function loadConfig() {
  ...
  try {
    ...
    return {
      maxAgeDays: Number.isFinite(parsed.maxAgeDays) && parsed.maxAgeDays >= 1
        ? parsed.maxAgeDays
        : DEFAULT_MAX_AGE_DAYS,
      maxSessions: Number.isFinite(parsed.maxSessions) && parsed.maxSessions >= 0
        ? parsed.maxSessions
        : DEFAULT_MAX_SESSIONS,
      maxTotalSizeMB: Number.isFinite(parsed.maxTotalSizeMB) && parsed.maxTotalSizeMB >= 0
        ? parsed.maxTotalSizeMB
...
export function apply(ctx) {
  ...
  async function pruneExpired() {
    ...
    for (const row of deletable) {
      const result = await removeSession(row.id)
      if (result.ok === true) deleted += 1
      else {
        failed += 1
  ...
  const runPrune = () => enqueue(() => pruneExpired())

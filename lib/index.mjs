...
//      动作：
//        delete          删除一个「冷」会话（非打开中、非运行中；子代理另须父
//                        会话已关闭）的全部持久化工件 —— JSONL 日志文件 + 空会话目录；
//                        成功后 emits
//                        `api-session/removed`，让 Web 端会话列表即时移除该行。
//        delete-expired  手动批量删除：删除「最近更新」早于 maxAgeDays 天的所有
//                        会话（跳过打开中 / 运行中 / 工件缺失 / 父仍打开的子代理），
//                        返回删除与跳过统计。
...
/** 会话日志工件名（代 + 物理编码）。官方规则（
 * @deepseek-ai/dsh-session-format/src/filename.ts sessionFormatLogFilename）：
 * v0 = session.jsonl，vN = session.vN.jsonl；zstd 编码再加 .zstd 后缀。
 * dsh 0.1.3 当前代是 v2、0.1.5 起是 v3（session.v3.jsonl.zstd），旧会话可能仍是
 * v0/v1/v2——固定文件名列表会漏掉这些代，删除变成空操作、快照也判成工件缺失。
 * 正则按代号泛化，因此 v3 及未来代无需再改。 */
const JSONL_ARTIFACT_RE = /^session(?:\.v([1-9][0-9]*))?\.jsonl(?:\.zstd)?$/

/** POSIX 写锁文件（官方 lease.ts:40 LEASE_FILENAME）：永不删除——删它会让后续加锁校验失去稳定 inode（lease.ts:118-123）。 */
const LEASE_FILENAME = 'session.lock'

/** 会话目录下的全部官方代工件名（不递归，未列目录/不存在返回空数组）。 */
async function artifactNamesIn(dir) {
  try {
    const names = await readdir(dir)
    return names.filter((name) => JSONL_ARTIFACT_RE.test(name))
  } catch {
    return []
  }
}
...
export function apply(ctx) {
   ...
   * 'no-location' 语义）。
   *
   *   - 官方 locate 可用（后端 JsonlSessionPersistence.locate）→ 取其返回的
   *     「当前代」路径；但该路径可能尚未物化（旧会话仍只有 v0/v1 工件），
   *     因此必须在会话目录里做存在性探测，命中任意代工件即返回它；
   *   - 官方 locate 不可用 → 按 <root>/<projectKey(cwd)|_no-cwd>/
   *     <encodeSegment(id)> 推导会话目录，探测全部官方代工件名。
   */
  // 注意：官方 locate() 是 JsonlSessionPersistence 的 private 方法（不在服务契约内），
  // 只当加速用；它只返回「当前代」路径，旧代会话靠 stat 失败后的目录探测兜底。
  async function locateOf(header) {
    let dir
    let official
    if (typeof sessionPersistence.locate === 'function') {
      official = sessionPersistence.locate(header)
      if (official !== undefined) {
        try {
          await stat(official.path)
          return official
        } catch {
          // 当前代工件未物化（旧代仍在）：落到目录探测
        }
        dir = dirname(official.path)
      }
    }
    if (dir === undefined) {
      try {
    ...
    const names = await artifactNamesIn(dir)
    return names.length === 0 ? undefined : { kind: 'jsonl', path: join(dir, names[0]) }
  }
  ...
  async function snapshotSessions() {
    ...
    for (const { header, snapshotSize } of entries) {
      ...
      if (live !== undefined) {
        ...
        try {
          ...
          // 会话格式迁移会把旧代工件留在同一目录（session.jsonl / .v1 / .v2 /
          // .v3 并存），而快照自带的 sizeBytes 只算当前代 → 阈值统计偏小。
          // 这里对该目录下的全部官方代工件求和；求和失败回退当前代字节数。
          let total = 0
          let counted = 0
          for (const name of await artifactNamesIn(dirname(location.path))) {
            try {
              total += (await stat(join(dirname(location.path), name))).size
              counted += 1
            } catch { /* 单个工件失败：按已统计部分求和 */ }
          }
          sizeBytes = counted > 0 ? total : (snapshotSize !== undefined ? snapshotSize : stats.size)
        } catch {
          artifactOk = false
        }
      }
      ...
      rows.push({
        ...
        parentClosed: !header.parentSession || !liveById.has(header.parentSession),
    ...
    for (const session of liveSessions) {
      ...
      rows.push({
        ...
        parentClosed: !session.header.parentSession || !liveById.has(session.header.parentSession),
  ...
  function statsOf(sessionsList, config) {
    ...
    // 与批量删除候选一致：子代理会话须父已关闭（parentClosed）才计入可删数
    const eligibleExpired = sessionsList.filter((row) =>
      (row.origin !== 'subagent' || row.parentClosed)
      && !row.live
      && !row.running
      && row.artifact
   ...
   *   - 子代理会话且父会话仍挂载（SessionStore 中）→ subagent-parent-live
   * 删除 = 清理会话目录下的全部官方代 JSONL 工件（session[.vN].jsonl[.zstd]，
   * 即 v0/v1/v2/v3 × zstd/明文）逐一 rm force + 尝试移除（已空的）会话目录；
   * 官方 POSIX 写锁文件 session.lock 永不删除（见 LEASE_FILENAME），因此
   * 「只剩 lock」时目录会保留（dirKept=true，仅内部字段，不透出到 HTTP 响应）。
   * 任何非预期失败逐项报告，不中断。
   */
  async function removeSession(id) {
    ...
    // 子代理会话额外要求父会话已关闭（父不在 SessionStore 中挂载即视为关闭）；
    // 父 id 缺失的孤儿不在此列，照常可删
    if (header.origin === 'subagent' && header.parentSession && sessions.get(header.parentSession) !== undefined) {
      return { error: 'subagent-parent-live', id }
    }
    const location = await locateOf(header)
    if (location === undefined) return { error: 'no-location', id }
    const dir = dirname(location.path)
    // 目录里的全部官方代工件都删（同一会话可能同时存在迁移前的 v0/v1 与当前
    // v2 文件）；只匹配官方文件名，绝不递归、绝不动其它文件。
    const artifacts = await artifactNamesIn(dir)
    if (artifacts.length === 0) return { error: 'no-location', id }
    try {
    ...
    // 显式判定残留：POSIX 侧官方在同一目录常驻写锁文件 session.lock
    // （lease.ts:40 LEASE_FILENAME，0 字节，永不删除——删它会让后续加锁校验
    // 失去稳定 inode，lease.ts:118-123），所以「删除成功、目录只剩 lock」是
    // 正常终态而非删除失败：内部返回 dirKept=true 并记一行日志（HTTP 响应
    // 形状不变，仍为 {ok:true,id}），人工排查时不再误判。
    let namesInDir = []
    try {
      namesInDir = await readdir(dir)
    } catch {
      /* 目录已不存在（外部清理）：按无残留处理 */
    }
    const remaining = namesInDir.filter((name) => name !== LEASE_FILENAME)
    const dirKept = remaining.length === 0 && namesInDir.includes(LEASE_FILENAME)
    if (dirKept) {
      ctx.logger.info(`[dsh-session-manager] session ${id} deleted; directory kept for ${LEASE_FILENAME}`)
    }
    try {
      await rmdir(dir)
    } catch {
    ...
    return { ok: true, id, dir, dirKept }
  }

  /**
   * 手动批量删除：删除「最近更新」早于 maxAgeDays 天的会话。
   * 候选：非子代理会话；或子代理会话且其父会话已关闭（快照行 parentClosed=true，
   * 父不在 SessionStore——父 id 缺失的孤儿视为已关闭）。
   * 跳过：live（打开中）、running（运行中）、工件缺失；以及已过期但父会话仍打开
   * 的子代理会话（计入 skipped.subagent —— 其生命周期随父会话走，父打开中自动
   * 跳过、父关闭后转为候选，与手动删除策略一致）。
   * 删除进度写入 pruneState（每删一个更新），供进度接口轮询。
  ...
  async function pruneExpired() {
    ...
    try {
      ...
      // 候选 = 已过期且（非子代理或父会话已关闭）；父仍打开的子代理在下方
      // skipped.subagent 单独计数，不进候选
      ...
      const skipped = {
        ...
        // 「父仍打开的子代理」数：已过期但父会话仍挂载（parentClosed=false），
        // 随父会话生命周期自动跳过，不参与本批删除
        subagent: rows.filter((row) => row.origin === 'subagent' && row.updatedAt < cutoff && !row.parentClosed).length,
